...
//echo $tc_Second.'<br>';                    
                        ...
                        $sql_booked_tktimetable = 'select * from tktimetable where tt_Guid="'.$tc_booked_TimeTable_Guid.'" and tt_Date >= CURDATE() ORDER BY DAYOFMONTH(tt_Date) DESC';
...
// Booked both online and offline, buggy
// $sql_classbooking = 'Select 
// tt_Guid, tt_Date,tt_Time_e,tt_Class,tt_Room,tt_Teacher,tt_Needbooking,tt_Keep,tt_Qty,
// ts_Member, ts_Date, tc_TimeTable_Guid, tc_ID
// from tkclassbooking
// LEFT OUTER JOIN tktimetable ON tc_TimeTable_Guid = tt_Guid
// LEFT OUTER JOIN tkswiping ON ts_TimeTable_Guid = tt_Guid AND ts_Member =tc_Member
// WHERE tc_Member = "'.$ct_No.'"
...
// ts_Member, ts_Date, tc_TimeTable_Guid, tc_ID
// from tkswiping
// LEFT OUTER JOIN tktimetable ON ts_TimeTable_Guid = tt_Guid
// LEFT OUTER JOIN tkclassbooking ON tc_TimeTable_Guid = tt_Guid AND tc_Member =ts_Member
// WHERE ts_Member = "'.$ct_No.'" AND tc_ID is null';
                        ...
                        $retval_classbooking = mysql_query( $sql_classbooking, $conn );                 
                            ...
                            $tt_Date = $row_classbooking['tt_Date'];
                            $tt_Date = $row_classbooking['tt_Date'];
                            $tt_Class = $row_classbooking['tt_Class'];
                            $tt_Room = $row_classbooking['tt_Room'];
                            $tt_Teacher = $row_classbooking['tt_Teacher'];

                            $the_real_tt_Date = date_create($row_classbooking['tt_Date']);
...
//echo $ts_TimeTable_Guid.'<br>';
                        ...
                        $retval_teacher = mysql_query( $sql_teacher, $conn );                 
                            ...
                            if($booked_status < $thedd && $ts_Date !='' || !empty($ts_Date) && $ts_TimeTable_Guid != '' && !empty($ts_TimeTable_Guid) ){
                            ...
                            elseif($booked_status < $thedd && $ts_TimeTable_Guid != '' && !empty($ts_TimeTable_Guid) ) {
                                $class_records_info .= '<tr class="diss">
                                                <td><span>日期</span>'.$booked_date.'</td>
                                                <td><span>時間</span>'.$booked_time.'</td>
                                                <td><span>課程</span>'.$cc_Name.'</td>

...
$category_types_Short = array();
                    //CONTENT
                        ...
                        while($row_cattypes = mysql_fetch_array($retval_cattypes, MYSQL_ASSOC)){
                            ...
                            $Beez[] = $row_cattypes['Short'];

                            $courseType .= '<option value="'.$catshort.'">'.$cattype.'</option>';


                        $sql_catbeez = 'select * from _types where Short IN ('.implode($Beez,',').') and Parent= 1';
                        $retval_catbeez = mysql_query( $sql_catbeez, $conn );
                             if(! $retval_catbeez )
                             {
                                die('Could not get data: ' . mysql_error());
                             }  

                             while($row_catbeez = mysql_fetch_array($retval_catbeez, MYSQL_ASSOC)){
                             $thebeez[] = $row_catbeez['Short']; 

                             }                 

                        }
//print_r(implode(array_unique($thebeez),','));
                        $sqlen_teachers = 'select * from cormember where ct_Type=02 and ct_No like "%D%" order by ct_Name ASC';
...
// }
                        ...
                        $sqlcategory_types = 'SELECT * FROM _types where Short != null';
                        ...
                        $sqlclasses = 'SELECT * FROM corclasses WHERE cc_No ="'.$classnumber.'" AND cc_ClassRoom ="'.$tt_Room.'"  order by cc_ClassRoom ASC ';// AND cc_Type IN ('.implode($category_types_Short,',').') AND cc_Type IN ('.implode(array_unique($thebeez),',').')
                        ...
                        //if( strtotime($tt_Time_e) > strtotime('now') ) { echo 'it has passed';}
                            ...
                            if( $now > strtotime($added_minutes) ){
                              $link = '<a class="class">';

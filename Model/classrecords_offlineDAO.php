...
// LEFT OUTER JOIN tkswiping ON ts_TimeTable_Guid = tt_Guid AND tc_Member = ts_Member WHERE tc_Member = "'.$ct_No.'" order by tc_ID desc
                        $sql_classbooking = 'Select tt_Date,tt_Time_e,tt_Class,tt_Room,tt_Teacher,tt_Needbooking,tt_Keep,tt_Qty,
ts_Member, ts_Date,
(SELECT tc_Guid FROM tkclassbooking WHERE tc_TimeTable_Guid = tt_Guid AND tc_Member = ts_Member) AS tc_Guid
from tktimetable
LEFT OUTER JOIN tkswiping ON ts_TimeTable_Guid = tt_Guid
WHERE ts_Member = "'.$ct_No.'" ORDER BY tt_Date desc ';

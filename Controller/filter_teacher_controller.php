...
if ($teacher_id != 'allTeachers') {
                        ...
                        //$sqlprojecten = 'SELECT * FROM tktimetable WHERE WEEKOFYEAR(tt_Date) = WEEKOFYEAR('.$week.') AND MONTH(tt_Date)=MONTH(NOW()) AND tt_Teacher="'.$teacher_id.'" ORDER BY tt_Date ASC ';
                        $sqlprojecten = 'SELECT * FROM tktimetable WHERE WEEKOFYEAR(tt_Date) = WEEKOFYEAR('.$week.') AND MONTH('.$week.')=MONTH('.$week.') AND tt_Teacher="'.$teacher_id.'" ORDER BY tt_Date ASC ';
                        ...
                        $sqlclasses = 'SELECT * FROM corclasses WHERE cc_No ="'.$classnumber.'" AND cc_Type IN ('.implode($category_types_Short,',').')   ';// AND cc_ClassRoom ="'.$tt_Room.'"
                        ...
                        $retvaltkClassBooking = mysql_query( $sqltkClassBooking, $conn );                 
                          ...
                          if($tc_ID > $tt_Qty){
                            ...
                            if($memberName != '' &&  $classname != ''  && $cc_Needbooking  != 1){
